...
public class CRC16M {
	...
	public static final String TAG = "CRC16M";
	...
	private static CRC16M crc16m;
	public CRC16M() {

		value = 0;

	}
	public static CRC16M getInstance(){
		if(crc16m == null){
			crc16m = new CRC16M();
		}
		crc16m.reset();
...
/*	private byte[] HexString2Buf(String src) {
	...
	public byte[] getSendBuf(byte[] bb) {
		...
		Log.d(TAG, "CRC16_1 ==> "+bb[0]);
		Log.d(TAG, "CRC16_2 ==> "+bb[1]);
		return getCRC16_4(bb);
	}
	private byte[] getCRC16_4(byte[] crc) {
		byte[] result = new byte[4];
		for (int d = 0; d < crc.length; d++) {
			byte i = crc[d];
			byte low = (byte) (i & 0xF);
			byte hi = (byte) ((i & 0xF0) >> 4);
			if (low < 10) {
				low += '0';
			} else {
				low = (byte) (low - 10 + 'A');
	...
	public static void main(String[] args) {
		...
		/*
		 * byte[] sbuf = CRC16M.getSendBuf(new byte[]{5,4,3,2,1}); int [] tt =
		 * {0xE5,0x05}; for(byte n : sbuf){ System.out.println(" >"+n); String s
		 * = Integer.toHexString((sbuf[0] & 0xFF) << 8 | sbuf[1] & 0xff); }
		 * byte[] tem = new byte[2]; tem[0]=sbuf[sbuf.length -2];
		 * tem[1]=sbuf[sbuf.length -1];
		 * 
		 * System.out.println(CRC16M.getBufHexStr(tem));
		 */
