...
public class AppraiseCommonData extends BaseData {
	...
	private static AppraiseCommonData commonData;
	private String hitMsg;
	private String appraiseResult;

	public static AppraiseCommonData getinstance() {
		if (commonData == null) {
			commonData = new AppraiseCommonData();
		}
		return commonData;
	}
	...
	public void setData(byte[] buffer, byte[] cmd) {
		...
		if (Arrays.equals(cmd, CMD.PJ)) {
			...
			String[] tellers = AssitTool.getSplit(arrayTeller,AssitTool.SPLIT_LINE_NEW);
			if (tellers != null) {
				if (tellers.length == 4) {
					setId(tellers[0]);
					setName(tellers[1]);
			...
			String[] options = AssitTool.getSplit(arrayOption,AssitTool.SPLIT_LINE_NEW);
			listButtonEn.clear();
			listButton.clear();
